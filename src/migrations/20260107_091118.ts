...
export async function up({ db, payload: _payload, req: _req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE \`jobs_required_documents\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`document\` text,
  ...
  `)
  await db.run(sql`CREATE INDEX \`jobs_required_documents_order_idx\` ON \`jobs_required_documents\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`jobs_required_documents_parent_id_idx\` ON \`jobs_required_documents\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`jobs\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`title\` text,
  	\`position\` text,
  	\`department\` text,
  	\`location\` text DEFAULT 'Parczew',
  ...
  `)
  await db.run(sql`CREATE INDEX \`jobs_meta_meta_image_idx\` ON \`jobs\` (\`meta_image_id\`);`)
  ...
  await db.run(sql`CREATE INDEX \`jobs_rels_media_id_idx\` ON \`jobs_rels\` (\`media_id\`);`)
  await db.run(sql`CREATE TABLE \`_jobs_v_version_required_documents\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`document\` text,
  ...
  `)
  await db.run(sql`CREATE INDEX \`_jobs_v_version_required_documents_order_idx\` ON \`_jobs_v_version_required_documents\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`_jobs_v_version_required_documents_parent_id_idx\` ON \`_jobs_v_version_required_documents\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`_jobs_v\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`parent_id\` integer,
  	\`version_title\` text,
  	\`version_position\` text,
  	\`version_department\` text,
  	\`version_location\` text DEFAULT 'Parczew',
  	\`version_employment_type\` text DEFAULT 'full-time',
  	\`version_salary\` text,
  	\`version_description\` text,
  	\`version_requirements\` text,
  	\`version_responsibilities\` text,
  	\`version_benefits\` text,
  	\`version_application_instructions\` text,
  	\`version_contact_person\` text,
  ...
  await db.run(sql`CREATE INDEX \`_jobs_v_version_meta_version_meta_image_idx\` ON \`_jobs_v\` (\`version_meta_image_id\`);`)
  ...
  await db.run(sql`CREATE INDEX \`_jobs_v_rels_media_id_idx\` ON \`_jobs_v_rels\` (\`media_id\`);`)
  await db.run(sql`CREATE TABLE \`public_procurements_cpv_codes\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`code\` text,
  	\`description\` text,
  	FOREIGN KEY (\`_parent_id\`) REFERENCES \`public_procurements\`(\`id\`) ON UPDATE no action ON DELETE cascade
  );
  `)
  await db.run(sql`CREATE INDEX \`public_procurements_cpv_codes_order_idx\` ON \`public_procurements_cpv_codes\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`public_procurements_cpv_codes_parent_id_idx\` ON \`public_procurements_cpv_codes\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`public_procurements_attachments\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`name\` text,
  ...
  `)
  await db.run(sql`CREATE INDEX \`public_procurements_attachments_order_idx\` ON \`public_procurements_attachments\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`public_procurements_attachments_parent_id_idx\` ON \`public_procurements_attachments\` (\`_parent_id\`);`)
  await db.run(sql`CREATE INDEX \`public_procurements_attachments_file_idx\` ON \`public_procurements_attachments\` (\`file_id\`);`)
  await db.run(sql`CREATE TABLE \`public_procurements\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`title\` text,
  	\`procurement_number\` text,
  	\`procedure_type\` text DEFAULT 'basic',
  	\`publish_date\` text,
  	\`deadline_date\` text,
  	\`opening_date\` text,
  	\`estimated_value\` text,
  	\`eu_project\` integer DEFAULT false,
  	\`description\` text,
  	\`contact_person\` text,
  	\`contact_phone\` text,
  	\`contact_email\` text,
  ...
  `)
  await db.run(sql`CREATE INDEX \`public_procurements_meta_meta_image_idx\` ON \`public_procurements\` (\`meta_image_id\`);`)
  await db.run(sql`CREATE UNIQUE INDEX \`public_procurements_slug_idx\` ON \`public_procurements\` (\`slug\`);`)
  await db.run(sql`CREATE INDEX \`public_procurements_updated_at_idx\` ON \`public_procurements\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`public_procurements_created_at_idx\` ON \`public_procurements\` (\`created_at\`);`)
  await db.run(sql`CREATE INDEX \`public_procurements__status_idx\` ON \`public_procurements\` (\`_status\`);`)
  await db.run(sql`CREATE TABLE \`_public_procurements_v_version_cpv_codes\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`code\` text,
  	\`description\` text,
  	\`_uuid\` text,
  	FOREIGN KEY (\`_parent_id\`) REFERENCES \`_public_procurements_v\`(\`id\`) ON UPDATE no action ON DELETE cascade
  );
  `)
  await db.run(sql`CREATE INDEX \`_public_procurements_v_version_cpv_codes_order_idx\` ON \`_public_procurements_v_version_cpv_codes\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`_public_procurements_v_version_cpv_codes_parent_id_idx\` ON \`_public_procurements_v_version_cpv_codes\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`_public_procurements_v_version_attachments\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`name\` text,
  ...
  `)
  await db.run(sql`CREATE INDEX \`_public_procurements_v_version_attachments_order_idx\` ON \`_public_procurements_v_version_attachments\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`_public_procurements_v_version_attachments_parent_id_idx\` ON \`_public_procurements_v_version_attachments\` (\`_parent_id\`);`)
  await db.run(sql`CREATE INDEX \`_public_procurements_v_version_attachments_file_idx\` ON \`_public_procurements_v_version_attachments\` (\`file_id\`);`)
  await db.run(sql`CREATE TABLE \`_public_procurements_v\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`parent_id\` integer,
  	\`version_title\` text,
  	\`version_procurement_number\` text,
  	\`version_procedure_type\` text DEFAULT 'basic',
  	\`version_publish_date\` text,
  	\`version_deadline_date\` text,
  	\`version_opening_date\` text,
  	\`version_estimated_value\` text,
  	\`version_eu_project\` integer DEFAULT false,
  	\`version_description\` text,
  	\`version_contact_person\` text,
  	\`version_contact_phone\` text,
  	\`version_contact_email\` text,
  	...
  	\`latest\` integer,
  	\`autosave\` integer,
  	FOREIGN KEY (\`parent_id\`) REFERENCES \`public_procurements\`(\`id\`) ON UPDATE no action ON DELETE set null,
  	FOREIGN KEY (\`version_meta_image_id\`) REFERENCES \`media\`(\`id\`) ON UPDATE no action ON DELETE set null
  );
  `)
  await db.run(sql`CREATE INDEX \`_public_procurements_v_parent_idx\` ON \`_public_procurements_v\` (\`parent_id\`);`)
  await db.run(sql`CREATE INDEX \`_public_procurements_v_version_meta_version_meta_image_idx\` ON \`_public_procurements_v\` (\`version_meta_image_id\`);`)
  await db.run(sql`CREATE INDEX \`_public_procurements_v_version_version_slug_idx\` ON \`_public_procurements_v\` (\`version_slug\`);`)
  await db.run(sql`CREATE INDEX \`_public_procurements_v_version_version_updated_at_idx\` ON \`_public_procurements_v\` (\`version_updated_at\`);`)
  await db.run(sql`CREATE INDEX \`_public_procurements_v_version_version_created_at_idx\` ON \`_public_procurements_v\` (\`version_created_at\`);`)
  await db.run(sql`CREATE INDEX \`_public_procurements_v_version_version__status_idx\` ON \`_public_procurements_v\` (\`version__status\`);`)
  await db.run(sql`CREATE INDEX \`_public_procurements_v_created_at_idx\` ON \`_public_procurements_v\` (\`created_at\`);`)
  await db.run(sql`CREATE INDEX \`_public_procurements_v_updated_at_idx\` ON \`_public_procurements_v\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`_public_procurements_v_latest_idx\` ON \`_public_procurements_v\` (\`latest\`);`)
  await db.run(sql`CREATE INDEX \`_public_procurements_v_autosave_idx\` ON \`_public_procurements_v\` (\`autosave\`);`)
  ...
  await db.run(sql`CREATE TABLE \`payload_locked_documents_rels\` (
  	...
  	FOREIGN KEY (\`public_procurements_id\`) REFERENCES \`public_procurements\`(\`id\`) ON UPDATE no action ON DELETE cascade,
  	FOREIGN KEY (\`media_id\`) REFERENCES \`media\`(\`id\`) ON UPDATE no action ON DELETE cascade,
  	FOREIGN KEY (\`categories_id\`) REFERENCES \`categories\`(\`id\`) ON UPDATE no action ON DELETE cascade,
  	FOREIGN KEY (\`users_id\`) REFERENCES \`users\`(\`id\`) ON UPDATE no action ON DELETE cascade,
  ...
  await db.run(sql`CREATE INDEX \`payload_locked_documents_rels_public_procurements_id_idx\` ON \`payload_locked_documents_rels\` (\`public_procurements_id\`);`)
...
export async function down({ db, payload: _payload, req: _req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`DROP TABLE \`jobs_required_documents\`;`)
  await db.run(sql`DROP TABLE \`jobs\`;`)
  await db.run(sql`DROP TABLE \`jobs_rels\`;`)
  await db.run(sql`DROP TABLE \`_jobs_v_version_required_documents\`;`)
  await db.run(sql`DROP TABLE \`_jobs_v\`;`)
  await db.run(sql`DROP TABLE \`_jobs_v_rels\`;`)
  await db.run(sql`DROP TABLE \`public_procurements_cpv_codes\`;`)
  await db.run(sql`DROP TABLE \`public_procurements_attachments\`;`)
  await db.run(sql`DROP TABLE \`public_procurements\`;`)
  await db.run(sql`DROP TABLE \`_public_procurements_v_version_cpv_codes\`;`)
  await db.run(sql`DROP TABLE \`_public_procurements_v_version_attachments\`;`)
  await db.run(sql`DROP TABLE \`_public_procurements_v\`;`)
